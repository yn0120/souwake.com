<!-- Menu -->
<aside id="layout-menu" class="layout-menu menu-vertical menu" data-bs-theme="dark" style="touch-action: none; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">
    ...
    <ul class="menu-inner py-1 ps ps--active-y">
        ...
        @if (preg_grep('/^officeRole.*$/', Auth::user()->routes()))
            <li class="menu-item {{ request()->route()->named('*officeRole*') ? 'active open' : '' }}">
                ...
                <ul class="menu-sub">
                    ...
                    @if (in_array('officeRoleCreate*', Auth::user()->routes()))
                        <li class="menu-item {{ request()->route()->named('officeRoleCreate*') ? 'active' : '' }}">
                            <a href="{{ route('officeRoleCreateInput', [], false) }}" class="menu-link">
                                登録
                            </a>
                        </li>
                    @endif
                    @if (in_array('officeRoleRouteEdit*', Auth::user()->routes()))
                        <li class="menu-item {{ request()->route()->named('officeRoleRouteEdit*') ? 'active' : '' }}">
                            <a href="{{ route('officeRoleRouteEditInput', [], false) }}" class="menu-link">
                                付与
                            </a>
                        </li>
                    @endif
        ...
        @if (preg_grep('/^officeAdmin.*$/', Auth::user()->routes()))
            <li class="menu-item {{ request()->route()->named('*officeAdmin*') ? 'active open' : '' }}">
                <a href="javascript:void(0);" class="menu-link menu-toggle">
                ...
                <ul class="menu-sub">
                    ...
                    @if (in_array('officeAdminCreate*', Auth::user()->routes()))
                        <li class="menu-item {{ request()->route()->named('officeAdminCreate*') ? 'active' : '' }}">
                            <a href="{{ route('officeAdminCreateInput', [], false) }}" class="menu-link">
                                登録
                            </a>
                        </li>
                    @endif
        ...
        <div class="ps__rail-y" style="top: 0px; height: 509px; right: 4px;">
