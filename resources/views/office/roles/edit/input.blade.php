<x-office.layout title="権限編集">
    <x-office.card title="権限編集">
        <form method="POST" enctype="multipart/form-data"
              action="{{ App\Libraries\Utils::urlToPath(route('officeRoleEditConfirm', ['id' => $assign['record']->id])) }}">
            @csrf
            ...
            <x-office.form.row label="備考" for="note" name="note">
                <x-office.form.textarea name="note" id="note" class="autoHeight">{{ old('note', $assign['record']->note) }}</x-office.form.textarea>
            </x-office.form.row>
