...
@section('meta')
    <title>出欠回答編集完了 | {{ config('app.name') }}</title>
@endsection
                            ...
                            <div class="card p-5">
                                <div class="row">
                                    <div class="col-12 pt-2">
                                        <h5 class="card-title">出欠回答編集完了</h5>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-12">
                                        <p class="mt-3">
                                            <div class="text-break w-100">出欠回答を編集しました。</div>
                                        </p>
                                    </div>
                                    @if (in_array('officeWeddingRsvpIndex*', Auth::user()->routes()))
                                        <div class="col-auto pb-2">
                                            <a href="{{ route('officeWeddingRsvpIndex', session('officeWeddingRsvpIndexSearchParams')) }}" class="btn btn-primary">出欠回答一覧</a>
                                        </div>
                                    @endif
                                    @if (in_array('officeWeddingRsvpShow*', Auth::user()->routes()))
                                        <div class="col-auto pb-2">
                                            <a href="{{ route('officeWeddingRsvpShow', ['id' => $assign['id']]) }}" class="btn btn-primary">出欠回答詳細</a>
                                        </div>
                                    @endif
                                </div>
                            </div>
