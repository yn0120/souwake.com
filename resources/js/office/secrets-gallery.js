 ...
 * 非公開ファイルの一覧とビューア。
 *
 * 見た目は専用CSS（旧 public/assets/css/secrets-gallery.css）を廃止し、
 * ここで付けるTailwindのユーティリティクラスと、Blade側の任意バリアント
 * （[&_.plyr__controls]:bottom-6 など）だけで表現している。
 */
(function () {
    'use strict';

    // 一覧の各行
    var ROW_CLASS = 'border-b border-default last:border-b-0';
    var LINK_CLASS = 'flex cursor-pointer items-center justify-between px-1 py-3 text-sm '
        + 'text-heading no-underline hover:bg-neutral-secondary';
    var DATE_CLASS = 'ml-4 shrink-0 whitespace-nowrap text-xs text-body';

    // ビューア内の画像・動画。ジェスチャーはステージ側で拾うため pointer-events は切る。
    // [-webkit-touch-callout:none] は長押しの保存メニューを出しにくくするため（完全な防止は不可）。
    var MEDIA_CLASS = 'pointer-events-none max-h-full max-w-full select-none touch-none';
    var IMAGE_CLASS = MEDIA_CLASS + ' will-change-transform [-webkit-touch-callout:none]';
    var VIDEO_CLASS = MEDIA_CLASS + ' h-full';

    // 動画の10秒送り/戻しヒント
    var SEEK_HINT_CLASS = 'pointer-events-none absolute top-1/2 z-[20015] flex size-16 '
        + '-translate-y-1/2 items-center justify-center rounded-full bg-black/40 text-3xl '
    ...
    function showSeekHint(side) {
        ...
        hint.className = SEEK_HINT_CLASS + (side === 'left' ? ' left-8' : ' right-8') + ' opacity-100';
