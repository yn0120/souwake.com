...
(function () {
    ...
    var currentParams = {};
    var showAlert = createToast('pwm-alert', 'pwm-alert-message');
    ...
    function buildValueFieldHtml(type, value, inputClass, placeholder) {
        ...
        if (type === 'textarea') {
            return '<textarea class="' + INPUT_CLASS + ' ' + inputClass + '" rows="3"' + placeholderAttr + '>' + esc(value || '') + '</textarea>';
        }

        return '<input type="' + htmlInputType(type) + '" class="' + INPUT_CLASS + ' ' + inputClass + '" value="' + esc(value || '') + '"' + placeholderAttr + '>';
    }
    ...
    // コピーボタン。clipboard.jsを使っていたが、navigator.clipboardで足りるため依存を外した。
    // 一覧は再描画されるためイベント委譲で拾う（描画ごとの再バインドが不要）。
    listEl.addEventListener('click', function (e) {
        var button = e.target.closest('.pwm-copy-btn');
        if (!button) {
            return;
        }
        navigator.clipboard.writeText(button.getAttribute('data-clipboard-text') || '').then(function () {
            showAlert('success', 'コピーしました。');
        }).catch(function () {
            showAlert('danger', 'コピーできませんでした。');
        });
    });
    ...
    function renderEntryCard(entry) {
        ...
        return ''
            ...
            + '      <div class="text-xs text-body">表示順: <span class="pwm-entry-order">' + esc(entry.display_order) + '</span></div>'
            + '    </div>'
            + '    <div class="pwm-entry-edit grow" style="display:none; max-width: 260px;">'
            + '      <input type="text" class="' + INPUT_CLASS + ' mb-2 pwm-entry-name-input" value="' + esc(entry.name) + '">'
            + '      <input type="number" class="' + INPUT_CLASS + ' pwm-entry-order-input" value="' + esc(entry.display_order) + '">'
            + '    </div>'
            + '    <div class="mt-2 ml-3 flex flex-wrap gap-2 whitespace-nowrap md:mt-0">'
            + '      <button type="button" class="' + BUTTON_CLASS.warning + ' pwm-entry-edit-btn">編集</button>'
            + '      <button type="button" class="' + BUTTON_CLASS.success + ' pwm-entry-update-btn" style="display:none;">更新</button>'
            + '      <button type="button" class="' + BUTTON_CLASS.dark + ' pwm-entry-cancel-btn" style="display:none;">キャンセル</button>'
            + '      <button type="button" class="' + BUTTON_CLASS.danger + ' pwm-entry-delete-btn">削除</button>'
            + '    </div>'
            + '  </div>'
            + '  <div class="mt-3">' + itemsHtml + '</div>'
            + '  <div class="mt-2 border-t border-default pt-2">'
            + '    <div class="grid grid-cols-12 items-center gap-2">'
            + '      <div class="col-span-6 md:col-span-3"><input type="text" class="' + INPUT_CLASS + ' pwm-new-item-label" placeholder="項目名"></div>'
            + '      <div class="col-span-6 md:col-span-2"><select class="' + INPUT_CLASS + ' pwm-new-item-type">' + itemTypeOptions('text') + '</select></div>'
            + '      <div class="col-span-8 md:col-span-5 pwm-new-item-value-field">' + buildValueFieldHtml('text', '', 'pwm-new-item-value', '値') + '</div>'
            + '      <div class="col-span-4 md:col-span-2"><button type="button" class="' + BUTTON_CLASS.secondary + ' w-full pwm-item-add-btn">＋ 項目を追加</button></div>'
            + '    </div>'
            + '  </div>'
            + '</div>';
    }
    ...
    function renderItemValueView(item) {
        if (item.type === 'textarea') {
            return ''
                + '<div class="whitespace-pre-wrap break-words text-sm text-heading pwm-item-value-display">' + esc(item.value || '') + '</div>'
                + '<button type="button" class="' + BUTTON_CLASS.link + ' pwm-copy-btn" data-clipboard-text="' + esc(item.value || '') + '">コピー</button>';
        }

        return ''
            + '<span class="break-words text-sm text-heading pwm-item-value-display">' + esc(item.value || '') + '</span> '
            + '<button type="button" class="' + BUTTON_CLASS.link + ' ml-1 pwm-copy-btn" data-clipboard-text="' + esc(item.value || '') + '">コピー</button>';
    }
    ...
    function renderItemRow(entryId, item) {
        ...
        return ''
            ...
            + '    <div class="col-span-12 flex flex-wrap gap-2 md:col-span-3 md:justify-end">'
            + '      <button type="button" class="' + BUTTON_CLASS.warning + ' pwm-item-edit-btn">編集</button>'
            + '      <button type="button" class="' + BUTTON_CLASS.danger + ' pwm-item-delete-btn">削除</button>'
            + '    </div>'
            + '  </div>'
            + '  <div class="grid grid-cols-12 items-center gap-2 pwm-item-edit" style="display:none;">'
            + '    <div class="col-span-12 md:col-span-3"><input type="text" class="' + INPUT_CLASS + ' pwm-item-label-input" value="' + esc(item.label) + '"></div>'
            + '    <div class="col-span-12 md:col-span-2"><select class="' + INPUT_CLASS + ' pwm-item-type-select">' + itemTypeOptions(item.type) + '</select></div>'
            + '    <div class="col-span-12 md:col-span-4 pwm-item-value-field">' + valueInput + '</div>'
            + '    <div class="col-span-12 flex flex-wrap gap-2 md:col-span-3 md:justify-end">'
            + '      <button type="button" class="' + BUTTON_CLASS.success + ' pwm-item-update-btn">更新</button>'
            + '      <button type="button" class="' + BUTTON_CLASS.dark + ' pwm-item-cancel-btn">キャンセル</button>'
            + '    </div>'
            + '  </div>'
            + '</div>';
    }
    ...
    function newCreateItemRow() {
        ...
        row.className = 'mb-2 grid grid-cols-12 items-center gap-2 pwm-create-item-row';
        row.innerHTML = ''
            + '<div class="col-span-6 md:col-span-3"><input type="text" class="' + INPUT_CLASS + ' pwm-create-item-label" placeholder="項目名（例: ログインID）"></div>'
            + '<div class="col-span-6 md:col-span-2"><select class="' + INPUT_CLASS + ' pwm-create-item-type">' + itemTypeOptions('text') + '</select></div>'
            + '<div class="col-span-8 md:col-span-5 pwm-create-item-value-field">' + buildValueFieldHtml('text', '', 'pwm-create-item-value', '値') + '</div>'
            + '<div class="col-span-4 md:col-span-2"><button type="button" class="' + BUTTON_CLASS.danger + ' w-full pwm-create-item-remove">削除</button></div>';
