document.addEventListener("DOMContentLoaded", function(e) {
    ...
    let i = document.querySelector("#horizontalBarChart")
      , l = {
        chart: {
            height: 360,
            type: "bar",
        ...
        dataLabels: {
            ...
            formatter: function(e, a) {
                return l.labels[a.dataPointIndex]
            },
            offsetX: 0,
            dropShadow: {
                enabled: !1
    ...
    let c = document.querySelector(".datatables-academy-course")
      , d = {
        angular: '<span class="badge bg-label-danger rounded p-1_5"><i class="icon-base bx bxl-angular icon-28px"></i></span>',
        figma: '<span class="badge bg-label-warning rounded p-1_5"><i class="icon-base bx bxl-figma icon-28px"></i></span>',
        ...
        fundamentals: '<span class="badge bg-label-primary rounded p-1_5"><i class="icon-base bx bx-diamond icon-28px"></i></span>'
    };
    c && ((t = document.createElement("h5")).classList.add("card-title", "mb-0", "text-nowrap", "text-md-start", "text-center"),
    t.innerHTML = "Course you are taking",
    new DataTable(c,{
        ...
        columnDefs: [{
            ...
            responsivePriority: 2,
            render: (e, a, s) => {
                let {logo: t, course: o, user: r, image: n} = s;
                s = n ? `<img src="${assetsPath}img/avatars/${n}" alt="Avatar" class="rounded-circle">` : `<span class="avatar-initial rounded-circle bg-label-${(s = ["success", "danger", "warning", "info", "dark", "primary", "secondary"])[Math.floor(Math.random() * s.length)]}">${(r.match(/\b\w/g) || []).reduce( (e, a) => e + a.toUpperCase(), "")}</span>`;
                return `
            ...
            render: e => {
                var e = moment.duration(e)
                  , a = Math.floor(e.asHours());
                return `<span class="fw-medium text-nowrap text-heading">${a + `h ${Math.floor(e.asMinutes()) - 60 * a}m`}</span>`
            }
        }, {
            targets: 4,
            render: (e, a, s) => {
                var {status: s, number: t} = s;
                return `
            ...
            targets: 5,
            render: (e, a, s) => {
                var {user_number: s, note: t, view: o} = s;
                return `
        ...
        responsive: {
            details: {
                ...
                renderer: function(e, a, s) {
                    var t, o, r, s = s.map(function(e) {
                        return "" !== e.title ? `<tr data-dt-row="${e.rowIndex}" data-dt-column="${e.columnIndex}">
                      <td>${e.title}:</td>
                      <td>${e.data}</td>
                    </tr>` : ""
                    }).join("");
                    return !!s && ((t = document.createElement("div")).classList.add("table-responsive"),
                    o = document.createElement("table"),
                    t.appendChild(o),
                    o.classList.add("table"),
                    o.classList.add("datatables-basic"),
    ...
    setTimeout( () => {
        [{
            ...
            document.querySelectorAll(e).forEach(a => {
                s.split(" ").forEach(e => a.classList.remove(e)),
                t && a.classList.add(t)
            }
            )
        }
