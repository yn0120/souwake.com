...
/**
 * ファイル機能のマスターキー（SECRETS_MASTER_KEY）を生成し .env に書き込む。
...
class SecretsKeyGenerateCommand extends Command
{
    ...
    protected $description = 'ファイル機能のマスターキー(SECRETS_MASTER_KEY)を生成する';
    ...
    public function handle(): int
    {
        ...
        if ($hasExisting && trim($matches[1] ?? '') !== '' && ! $this->option('force')) {
            ...
            $this->warn('既存のキーを上書きすると、それまでに暗号化されたファイルは復号できなくなります。事前に secrets:rewrap の実行を検討してください。');
